...
import { sanitizeExecutionCommand, sanitizeExecutionOutput, MAX_OUTPUT_LINES } from "../src/execution/sanitize.js";
...
describe("sanitizeExecutionOutput", () => {
  ...
  it("redacts home paths from released command labels too", () => {
    const command = sanitizeExecutionCommand(String.raw`C:\Users\alice\.cache\codex\pwsh.exe -Command "node tests/smoke.js"`);
    expect(command).not.toContain("C:\\Users\\alice");
    expect(command).toContain(String.raw`C:\Users\[user]`);
  });
