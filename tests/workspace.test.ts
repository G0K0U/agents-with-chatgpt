...
describe("path containment", () => {
  ...
  it("rejects a Windows junction escaping the workspace", () => {
    if (process.platform !== "win32") return;
    const junction = path.join(root, "junction-out");
    try {
      fs.symlinkSync(outside, junction, "junction");
    } catch {
      return; // Junction creation may be disabled by the test runner policy.
    }
    try {
      expect(() => ws.resolve("junction-out/secret.txt")).toThrowError(WorkspaceError);
      try {
        ws.resolve("junction-out/secret.txt");
        expect.unreachable("junction escape should be rejected");
      } catch (error) {
        expect((error as WorkspaceError).code).toBe("PATH_OUTSIDE_WORKSPACE");
      }
    } finally {
