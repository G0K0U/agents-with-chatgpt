...
export interface OAuthDeps {
  ...
  /** Names only; canonical filesystem paths never enter the OAuth UI. */
...
function pairingPage(opts: {
  ...
  const scopeLabels: Record<string, string> = {
    ...
    "execution.submit": "Submit scoped coding tasks to Codex",
    "execution.cancel": "Cancel submitted Codex tasks",
    "execution.queue": "Pause or resume a workspace task queue",
    "audit_mirror.write": "Write only the fixed Engineering AI audit mirror",
  ...
  const scopeList = opts.scopes
    ...
    .join("");
  const workspaceList = [...new Set(opts.workspaceNames ?? [opts.workspaceName])]
...
<div class="card">
  ...
  <p class="sub">ChatGPT is requesting scoped access to these pre-authorized local workspaces:</p>
  <ul>${workspaceList}</ul>
  <p class="sub">Requested capabilities:</p>
  <ul>${scopeList}</ul>
...
export function createOAuthRouter(deps: OAuthDeps): Router {
  ...
  router.get("/oauth/authorize", (req, res) => {
    ...
    res
      ...
      .send(pairingPage({
        ...
        workspaceNames: deps.workspaceNames,
        scopes,
      }));
  });
  ...
  router.post("/oauth/authorize", urlencoded({ extended: false }), (req, res) => {
    ...
    const code = deps.store.createAuthorizationCode({
      ...
      scopes: request.scopes,
      workspaceIds: deps.store.authorizedWorkspaceIds,
