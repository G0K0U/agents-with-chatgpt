...
export function endpointFile(workspaceId: string, stateDir?: string): string {
  return path.join(getStateDir(stateDir), "endpoints", `${workspaceId}.json`);
}

export function readLastEndpoint(workspaceId: string, stateDir?: string): LastEndpoint | null {
  return readJsonIfExists<LastEndpoint>(endpointFile(workspaceId, stateDir));
}

export function writeLastEndpoint(endpoint: Omit<LastEndpoint, "savedAt">, stateDir?: string): LastEndpoint {
  const saved: LastEndpoint = { ...endpoint, savedAt: new Date().toISOString() };
  writeSecureJson(endpointFile(saved.workspaceId, stateDir), saved);
  return saved;
}
