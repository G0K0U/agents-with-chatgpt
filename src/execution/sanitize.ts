...
/** Apply the same non-secret path/token redaction to a released command label. */
export function sanitizeExecutionCommand(command: string): string {
  return redactHomePaths(applyExtraRedact(redact(command))).slice(0, 200);
}
