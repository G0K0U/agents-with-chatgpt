...
export interface CloudflaredAccount {
  ...
  routeDns(tunnelName: string, hostname: string, opts?: { overwrite?: boolean }): Promise<void>;
}
...
export class ProcessCloudflaredAccount implements CloudflaredAccount {
  ...
  async routeDns(tunnelName: string, hostname: string, opts: { overwrite?: boolean } = {}): Promise<void> {
    const result = this.run([
      "tunnel",
      "route",
      "dns",
      ...(opts.overwrite === false ? [] : ["--overwrite-dns"]),
      tunnelName,
      hostname,
    ]);
    if (result.ok) return;
...
export async function provisionNamedTunnel(opts: {
  ...
  /** Explicit named requests fail closed instead of silently switching hosts. */
  ...
  try {
    ...
    // An existing CNAME may point at an older tunnel.  Treating that as
    // success is exactly how a registered fixed hostname can return 502.
    await account.routeDns(tunnel.name, hostname, { overwrite: true });
    ...
    if (opts.fallbackOnFailure === false) {
      return {
        ok: false,
        state: readExistingNamedOrUnsetState(opts.workspaceId),
        fallback: false,
...
function readExistingNamedOrUnsetState(workspaceId: string): TunnelState {
  const current = readTunnelState(workspaceId);
  if (current.preference === "named") return current;
