...
export function tunnelStateFile(workspaceId: string, stateDir?: string): string {
  return path.join(getStateDir(stateDir), "tunnels", `${workspaceId}.json`);
}

/** Machine-local files owned by the bridge for a named tunnel. */
export function tunnelRuntimeDir(workspaceId: string, stateDir?: string): string {
  return path.join(getStateDir(stateDir), "tunnels", workspaceId);
}

export function namedTunnelConfigFile(workspaceId: string, stateDir?: string): string {
  return path.join(tunnelRuntimeDir(workspaceId, stateDir), "cloudflared.yml");
}

export function namedTunnelRuntimeFile(workspaceId: string, stateDir?: string): string {
  return path.join(tunnelRuntimeDir(workspaceId, stateDir), "runtime.json");
}

export function readTunnelState(workspaceId: string, stateDir?: string): TunnelState {
  return (
    readJsonIfExists<TunnelState>(tunnelStateFile(workspaceId, stateDir)) ?? {
      workspaceId,
      preference: "unset",
    }
  );
...
export function writeTunnelState(state: TunnelState, stateDir?: string): TunnelState {
  writeSecureJson(tunnelStateFile(state.workspaceId, stateDir), state);
  return state;
}
...
export function isNamedTunnelReady(state: TunnelState): boolean {
  return (
    ...
    Boolean(state.tunnelId && TUNNEL_ID_RE.test(state.tunnelId)) &&
