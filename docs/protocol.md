...
The MCP bridge also exposes a deliberately narrow execution lifecycle. These
tools are separate from the existing read/review tools and require explicit
OAuth scopes:

| Tool | Scope | Purpose |
| --- | --- | --- |
| `submit_codex_task` | `execution.submit` | Queue one local Codex task through the selected deployment mode |
| `get_codex_task` | `execution.read` | Read bounded status and audit metadata for a task |
| `cancel_codex_task` | `execution.cancel` | Interrupt the active official Codex turn |
| `execution_queue` | `execution.read` for status; `execution.queue` for pause/resume | Inspect or persistently pause/resume one authorized workspace queue |
| `write_engineering_ai_audit_mirror` | `audit_mirror.write` | Atomically write bounded text to one of the two fixed Engineering AI OneDrive targets |

`execution_queue` always requires an explicit `workspace_id`. `action=status`
is read-only; `action=pause` and `action=resume` require the workspace-scoped
`execution.queue` scope. Pausing leaves a running task alone, holds queued tasks
in FIFO order, and persists under the bridge state directory so a controlled
restart cannot accidentally release the queue. The queue state is visible in
`workspace_info` and `execution_summary`.

The audit-mirror writer has no generic path or filesystem-read capability. It
accepts a bounded payload, or reads the matching fixed ledger through an
authorized Engineering AI workspace. The machine-local OneDrive root comes
only from the explicit `ENGINEERING_AI_AUDIT_MIRROR_ROOT` setting (or legacy
bridge setting), standard OneDrive environment variables, or bounded discovery
of the exact named account folder. Status reads `docs/audit-loop-state.md`,
while timeline reads `docs/audit-execution-timeline.md`. Each target is
realpath-checked, link/reparse-point checked, hashed, and recorded in the
workspace JSONL audit log.

The accepted envelope is conceptually:

```json
{
  "workspace_id": "<id from workspace_info>",
  "instruction": "<bounded coding request>",
  "write_scope": ["tests"],
  "network": false,
  "run_tests": true,
...
The task registry is the immutable source of network intent:
`networkRequested` is the original submission request and `networkEffective`
is the policy result for that task; `network` is only the compatibility alias
for `networkEffective`. `networkReported` may contain later runtime evidence,
but execution history cannot rewrite the requested/effective fields. For legacy
task JSON that lacks the explicit fields, bootstrap reconstructs deterministically
from `networkEffective`, then legacy `network`, then safe offline `false`, and
uses the effective value as the requested value when `networkRequested` is
absent. Conflicting historical or verification records are evidence only and
never silently replace task submission truth.

`run_tests=false` leaves verification disabled. With `run_tests=true`, the
bridge selects the registered profile for the connected workspace and runs its
fixed executable/argv after the coding turn through official App Server
`command/exec`; the request cannot provide a command, executable, cwd, sandbox
or network override. The profile is typed and bridge-owned, uses a bounded
timeout, and always has network disabled. Verification output is sanitized by
the existing output store and linked to the task. Its profile id, workspace id,
argv hash, cwd alias, sandbox, start/end times, exit code and status are included
in the task view and execution record. If the profile is missing, invalid, times
out or exits nonzero, the task is failed with a corresponding typed error.

The coding turn and deterministic verification are separate capabilities: the
turn receives the user-declared `write_scope`, while verification may read the
connected repository but writes only to a task-specific C2C runtime root. On
Windows this single external writable root avoids the official unelevated
sandbox's unsupported split-root case without disabling the sandbox. The bridge
holds its active slot until App Server/runtime cleanup finishes, and timeout,
approval denial, policy violation, completion and cancellation can publish only
one terminal record.

The old Computer Use `INIT → PLAN → EXECUTING → EXECUTED → REVIEW` loop remains
compatible for Codex-led sessions. A submitted task is the direct MCP path for
ChatGPT-led scoped execution; it does not add a second agent lifecycle.
...
14. If the user explicitly requests direct ChatGPT-led execution and the
