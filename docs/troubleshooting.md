...
For an explicit fixed host, add `--hostname <host>`. The bridge rewrites the
machine-local ingress with its current dynamic port on every start and checks
the public `/mcp` response before reporting success.
